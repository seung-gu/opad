...
**현재 상태:**
- `user_id` 필드가 이미 Article 모델에 존재하지만 항상 `None`
- MongoDB에 `user_id` 인덱스가 이미 준비되어 있음
- 중복 체크 로직이 `user_id`를 고려하도록 설계됨
...
```javascript
{
  "_id": "user-uuid",           // 사용자 ID (user_id로 사용)
...
#### Articles 컬렉션 (기존)
- `user_id` 필드 활용 (현재 `None`, 인증 후 사용자 ID로 설정)
- 기존 인덱스 그대로 사용 가능

#### Vocabularies 컬렉션 (확장 필요)
- 현재 `article_id`만 있음
- `user_id` 필드 추가 권장 (직접 조회 시 성능 향상)
...
**변경:**
- Vocabulary 저장/조회 시 `user_id` 필터링 추가
...
- 기존 `user_id=None`인 Article은 그대로 유지
- 로그인하지 않은 사용자는 여전히 Article 생성 가능 (user_id=None)
- 로그인한 사용자는 자신의 Article만 조회

**옵션 2: 기존 데이터를 특정 사용자에게 할당**
- 마이그레이션 스크립트로 기존 Article에 `user_id` 할당
- 복잡하고 위험함 (권장하지 않음)
...
- 로그인한 사용자는 `user_id` 설정, 비로그인은 `None`
...
7. ✅ Article 생성 시 `user_id` 자동 설정
8. ✅ Article 조회 시 `user_id` 필터링

**예상 작업 시간**: 2-3일

### Phase 2: 데이터 격리 강화
1. ✅ Vocabulary에 `user_id` 추가
2. ✅ 모든 CRUD 작업에 `user_id` 필터링
3. ✅ 통계 API 사용자별 필터링
