"""Tests for Free Dictionary API parsing utilities.

Tests cover language code mapping, gender extraction, phonetics, forms parsing,
and reflexive pronoun stripping with proper edge cases.

Note: TestExtractEntryMetadata and TestFetchFromFreeDictionaryApi were removed
because extract_entry_metadata() now returns GrammaticalInfo (not dict) and
fetch_from_free_dictionary_api() was replaced by FreeDictionaryAdapter.fetch().
"""
