...
### Added
- Centralized CSS variable theming system in `globals.css` for consistent design across all components
- Semantic color groups: vocab, system, danger, accent, warn for standardized component styling
- Reusable button classes (btn-primary, btn-outline) with modifier support for consistent button styling
- Enhanced `styleHelpers.ts` to use CSS variables for CEFR level colors (A1-C2 range)
- Improved ArticleStatusBadge component documentation

### Changed
- Removed duplicate CSS variables (accent2, accent3, mid) in favor of centralized semantic system
- Fixed Tailwind configuration duplication across component configs
- Updated 22+ component files to use centralized CSS variables for consistent theming
- Refactored color system for improved maintainability and type safety

## [0.8.1] - 2026-02-01

### Fixed
- CrewAI template variable error: ensure `vocabulary_list` is always set with default empty string to avoid "Missing required template variable" errors during article generation
- Correct parameter type in GET function for article usage API endpoint
